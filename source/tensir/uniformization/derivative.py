...
def log_likelihood_gradient_dataset(data, theta, theta_is_log, threads=1):
    """
...
@cached(cache={}, key=lambda d, T, l, t: hashkey(tuple(map(tuple, d)), tuple(T)))
def log_likelihood_gradient_dataset_cached(data, theta, theta_is_log, threads):
    """
    Like log_likelihood_gradient_dataset() but caches the result values of given (data, Theta) pairs.
    """
    return log_likelihood_gradient_dataset(data, theta, theta_is_log, threads)
